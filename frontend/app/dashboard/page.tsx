...
function SectorDivider({ label, accent = 'cyan' }: { label: string; accent?: string }) {
    const lines: Record<string, string> = { cyan: 'via-accent-cyan/15', green: 'via-accent-green/15', blue: 'via-accent-blue/15', orange: 'via-accent-orange/15' };
    const texts: Record<string, string> = { cyan: 'text-accent-cyan/30', green: 'text-accent-green/30', blue: 'text-accent-blue/30', orange: 'text-accent-orange/30' };
    return (
    ...
    return (
        <main className="relative min-h-screen bg-bg-primary text-text-primary selection:bg-accent-cyan/30 selection:text-white overflow-x-hidden">
            ...
            <div className="fixed inset-0 z-0 pointer-events-none">
                ...
                <div className="absolute top-0 left-0 w-full h-full" style={{ background: 'radial-gradient(ellipse 80% 50% at 20% 10%, rgba(0,153,255,0.04) 0%, transparent 70%)' }} />
                <div className="absolute top-0 right-0 w-full h-full" style={{ background: 'radial-gradient(ellipse 60% 40% at 80% 80%, rgba(0,245,255,0.03) 0%, transparent 70%)' }} />
            </div>

            <Navbar />

            <section id="dashboard"><HeroSection /></section>
            <SectorDivider label="CAPABILITIES · MODULE" accent="blue" />
            ...
            <footer className="relative z-10 border-t border-accent-cyan/8 py-8 px-4 mt-8">
                <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div className="terminal-text text-xs text-text-secondary/30 tracking-widest">ORBITAL TERRAIN INTELLIGENCE · ISRO AI PLATFORM</div>
